...
$config = @{
    "ENV" = "LOCALHOST";
    "Database" = "CRM";
    "DACPACPath" = "x:\!WORK\!Moje\2020-03-07 SQLSat Belgium\SQLDB";
    "Report" = "True";
    "Script" = "True";
    "Deploy" = "True";
...
#
# Publish with Publish Profile
#

# Generate the (differencial) script only
...
# Deploy database's changes to target server
Publish-DbaDacPackage -SqlInstance $config["TargetServer"] -Database $config["TargetDatabase"] `
    -Path $DACPACPackageName `
    -PublishXml $PublishProfile -OutputPath $OutputFolder
...
#error:
#Publish-DbaDacPackage : Parameter set cannot be resolved using the specified named parameters.
